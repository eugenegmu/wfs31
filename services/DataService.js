...
* */
const getFeature = ({ collectionId, featureId }) => new Promise(
  async (resolve, reject) => {
    let retdata=await Modeler.getFeature({collectionId, featureId});
    try {
...
* */
const getFeatures = ({ collectionId, limit, bbox, datetime }) => new Promise(
  async (resolve, reject) => {
    console.log("First CollectionId=");
    console.log(collectionId);
    let retdata=await Modeler.getFeatures({collectionId, limit, bbox, datetime});
    try {
      if (retdata.status_code == 200)
        resolve(Service.successResponse(retdata.payload,200));
      else {
