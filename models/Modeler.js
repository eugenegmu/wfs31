...
const getFeature = async ({ collectionId, featureId }) => {
  ...
  let payload=GetProviders.getProviders({collectionId});
  ...
  try {
    ...
    console.log("Going into getfeature");
    retdata=await mls.getFeature({providers, collectionId, featureId });
    console.log("In modeler - retdata=");
    console.log(retdata);
  }catch(excep_var){
    retdata.status=500;
    retdata.message='Internal Server Error.';
  }
  return retdata;
}
/**
...
* */
const getFeatures = async ({ collectionId, limit, bbox, datetime }) => {
  let retdata={
    'payload':{
        'features': [],
        'links': []
  ...
  try {
    ...
    retdata=await mls.getFeatures({providers, collectionId, limit, bbox, datetime });
  }catch(excep_var){
    retdata.status=500;
    retdata.message='Internal Server Error.';
  }
